...
def entrenarTagger():
    ...
    uni_tag = UnigramTagger(cess_sents)
...
def comenzar():
    ...
    tagger = entrenarTagger()

    tagged_tokens = tagger.tag(normalized_tokens) 

    new_tagged_tokens = []

    for first, second in tagged_tokens:
        if second != None:
            pair = (first, second[0].lower())
            new_tagged_tokens.append(pair)
        else:
            new_tagged_tokens.append((first,'no'))

    lemmatized_tokens = lemmatize(new_tagged_tokens, "C:/Users/sebas/Downloads/generate.txt")
